...
data = read_excel('trains_by_cause.xlsx')
...
i_external = which(data$main_cause=='External')
i_rolling = which(data$main_cause=='Rolling')
i_rail = which(data$main_cause=='Rail')
i_traffic = which(data$main_cause=='Traffic')
...
col_cause = rep('chartreuse2', n)
col_cause[i_rolling] = 'coral'
col_cause[i_rail] = 'dodgerblue'
col_cause[i_traffic] = 'purple'
...
cause = as.factor(data$main_cause)
#### Exploratory plots ####
x11()
plot(cause, avg_delay_arr)
x11()
plot(data$avg_delay_all_departing, data$avg_delay_all_arriving, col=col_cause, pch=16)
legend("topleft", legend=levels(cause), fill=unique(col_cause), cex=.7)
x11()
plot(data$journey_time_avg, data$avg_delay_all_arriving, col=col_cause, pch=16)
legend("topleft", legend=levels(cause), fill=unique(col_cause), cex=.7)
x11()
...
#### Parametric model ####
model_param = lm(avg_delay_arr ~ avg_delay_dep + avg_journey + year)
summary(model_param)
coefficients = model_param$coefficients
x11()
par(mfrow=c(2,2))
plot(model_param)
...
avg_dep_grid = seq(range(avg_delay_dep)[1],range(avg_delay_dep)[2], length.out = 100)
avg_journ_grid = seq(range(avg_journey)[1],range(avg_journey)[2], length.out = 100)
num_trips_grid = seq(range(num_trips)[1],range(num_trips)[2], length.out = 100)

open3d()
plot3d(avg_delay_dep[i_2015], avg_journey[i_2015], avg_delay_arr[i_2015], pch=16, col=col_years[i_2015])
lines3d(avg_dep_grid, avg_journ_grid, coefficients[1] + avg_dep_grid * coefficients[5] + avg_journ_grid * coefficients[9], lwd = 2)
open3d()
plot3d(avg_delay_dep[i_2016], avg_journey[i_2016], avg_delay_arr[i_2016], pch=16, col=col_years[i_2016])
lines3d(avg_dep_grid, avg_journ_grid, coefficients[1] + coefficients[2] + avg_dep_grid * coefficients[6]+ avg_journ_grid * coefficients[10], lwd = 2)
open3d()
plot3d(avg_delay_dep[i_2017], avg_journey[i_2017], avg_delay_arr[i_2017], pch=16, col=col_years[i_2017])
lines3d(avg_dep_grid, avg_journ_grid, coefficients[1] + coefficients[3] + avg_dep_grid * coefficients[7]+ avg_journ_grid * coefficients[11], lwd = 2)
open3d()
plot3d(avg_delay_dep[i_2018], avg_journey[i_2018], avg_delay_arr[i_2018], pch=16, col=col_years[i_2018])
lines3d(avg_dep_grid, avg_journ_grid, coefficients[1] + coefficients[4] + avg_dep_grid * coefficients[8]+ avg_journ_grid * coefficients[12], lwd = 2)


model_param = lm(avg_delay_arr ~ avg_delay_dep:year + avg_journey:year + year + num_trips)
summary(model_param)
