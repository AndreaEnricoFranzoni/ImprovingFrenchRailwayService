...
data = data[,-c(1,2, 9, 13, 17)]                                                # We clean the original data from useless columns (comments, month, year)
names(data)
na_indeces=NULL
for(i in 1:n)
    if(any(is.na(data[i,])))
...
data=na.omit(data)                                                              # we remove all NAs from the dataset, i.e. we drop single observations
...
for (i in 1:length(unique(data$route))){
  ...
  for (k in by_mean){
    tot = data_agg[i, 5]
    data_agg[i,k] =     sum(data[which(data$route==unique(data$route)[i]),5]*
                              data[which(data$route==unique(data$route)[i]),k])/tot
  }
  for (m in by_cause){
    tot = data_agg[i, 10]
    data_agg[i,m] =     sum(data[which(data$route==unique(data$route)[i]),10]*
...
#write_xlsx(data_agg, 'aggregated_trains.xlsx')
