...
#### Plausibilmente da non fare ####
x11()
hist(data$journey_time_avg)
i_short_2018 = which(multiv_2018$time<=120)
i_long_2018 = which(multiv_2018$time>120)

# interessante capire quali tratte brevi hanno ritardi sopra mezz'ora significativi (outliers)

x11()
plot(canc_perc[i_2018], extreme_delay[i_2018], col=ifelse(1:N%in%i_long_2018,"black","red"),pch=16,
     xlim=x_lim, ylim=y_lim)
#text(canc_perc[i_2018], extreme_delay[i_2018], label=1:N)

# 89 Macon Loche Paris Lyon
# 39 Lyon Marseille also in 2015 with >0.05, and also 2017
